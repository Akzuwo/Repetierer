...
function buildResultMessage({ student, assessment, round, sender }) {
	...
	const rows = round.criteria.map(criterion => {
		...
		return { label: criterion.label, self: formatRating(criterion, self), teacher: formatRating(criterion, teacher), difference: teacher - self };
	});
	...
	const text = [
		...
		...textRows,
		'',
		`Zusätzliche Qualität: ${assessment.studentAdditionalQuality || '–'}`,
		`Sonstige Bemerkungen: ${assessment.studentComment || '–'}`,
	...
	const html = `<!doctype html><html><body style="margin:0;background:#f3f4f5;color:#202125;font-family:Arial,sans-serif"><div style="max-width:680px;margin:0 auto;padding:28px"><div style="background:#05070b;color:#f5f5f5;border-radius:12px;padding:26px"><h1 style="margin:0 0 8px;color:#60a5fa;font-size:24px">Mitmachnote</h1><p style="margin:0 0 24px">${escapeHtml(round.title)} · ${escapeHtml(student.name)}</p><table style="border-collapse:collapse;width:100%;background:#fff;color:#202125;border-radius:8px;overflow:hidden"><thead><tr style="background:#2563eb;color:#fff"><th style="padding:10px;text-align:left">Kriterium</th><th style="padding:10px">Schüler</th><th style="padding:10px">Lehrer</th><th style="padding:10px">Differenz</th></tr></thead><tbody>${tableRows}</tbody></table><h2 style="font-size:16px;margin:24px 0 6px;color:#60a5fa">Zusätzliche Qualität</h2><p style="white-space:pre-wrap;margin:0">${escapeHtml(assessment.studentAdditionalQuality || '–')}</p><h2 style="font-size:16px;margin:20px 0 6px;color:#60a5fa">Sonstige Bemerkungen</h2><p style="white-space:pre-wrap;margin:0">${escapeHtml(assessment.studentComment || '–')}</p><h2 style="font-size:16px;margin:20px 0 6px;color:#60a5fa">Kommentar Lehrer</h2><p style="white-space:pre-wrap;margin:0">${escapeHtml(assessment.teacherComment || '–')}</p></div></div></body></html>`;
	return {
...
function formatRating(criterion, value) {
